...
/*   Updated: 2025/07/28 14:13:33 by rgodet           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../cub3d.h"

static void	render_lose_bg(t_game *game)
{
...
void	put_retry_to_lose(t_game *game)
{
...
void	put_menu_to_lose(t_game *game)
{
...
void	put_exit_to_lose(t_game *game)
{
...
void	render_lose(t_game *game)
{
	...
	mlx_put_transformed_image_to_window(game->graphics->init,
		game->graphics->window, game->img[GHOSTED_TITLE],
		(WIDTH_WINDOW - (903 * scale)) / 2,
		((HEIGHT_WINDOW - (184 * scale)) / 2) - 350,
		scale, scale, 0.0f);
	put_retry_to_lose(game);
	put_menu_to_lose(game);
	put_exit_to_lose(game);
